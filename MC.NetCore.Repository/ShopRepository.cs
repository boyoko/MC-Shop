    ...
    public class ShopRepository : Repository<Shop>, IShopRepository
    {
        ...
        public async Task<IList<ShopDto>> GetShopList()
        {
            ...
            try
            {
                var query = await (from shop in _shopDbContext.Shop.AsNoTracking()
                                   join bat in ( from single in  _shopDbContext.GoodsPickingBatch
                                                 where single.PickupType == null || single.PickupType == 1
                                                 select single)
        ...
        private decimal GetTotalSale(ShopDto dto)
        {
            var query = (from m in _shopDbContext.OrderForm
                         ...
                         where m.Status==4
                         && m.ShopId==dto.ShopId
                         && m.OrderTime>=dto.PickTime
                         && m.OrderTime <= DateTime.Now
